...
if __name__ == "__main__":
    ...
    eloEnroque = chess.map(lambda line: (line[15], line[14]))


    #(cantidad de enroques, cantidad de ELO distintos)
    nEloEnroque = eloEnroque.aggregateByKey((0.0, 0), \
    ...
    percentageEloEnroque = nEloEnroque.mapValues(lambda tup2n: tup2n[0]/tup2n[1]*100)

    df = spark.createDataFrame(percentageEloEnroque, ['elo', 'porcentajeEnroque'])

    percentageEloEnroqueSorted = df.sort(['porcentajeEnroque'],ascending = [False])

    top10EloEnroque = percentageEloEnroqueSorted.limit(10)
    top10EloEnroque.show(10)

    top10EloEnroque.saveAsTextFile(fileout)
