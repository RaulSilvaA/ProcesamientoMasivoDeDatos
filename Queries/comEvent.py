...
if __name__ == "__main__":
    ...
    chess = lines.map(lambda line: (str(line[1]).strip(), line[2]+'#'+line[3], line[4], line[5], line[6], \
         line[7], line[8], line[9], line[10], line[11], line[11].count('x')))
    

    #(Event, n_piezas_comidas)
    comEvent =  chess.map(lambda line: (line[0], line[10]))
    ...
    cAvgComEvent = avgComEvent.coalesce(1)

    cAvgComEvent.saveAsTextFile(fileout)
